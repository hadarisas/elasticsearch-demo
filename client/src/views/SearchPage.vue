...
const navigateToPrevious = () => {
  if (currentExample.value > 1) {
    currentExample.value--;
    // Add your navigation logic here
    // You might want to use router.push or emit an event
  }
};

const navigateToNext = () => {
  if (currentExample.value < totalExamples) {
    currentExample.value++;
    // Add your navigation logic here
    // You might want to use router.push or emit an event
  }
    ...
    >
      <div :class="searchResults.hits.length ? '' : '-mt-32'">
        <h1 class="text-center mb-8">
          <span class="text-4xl font-extrabold flex justify-center">
            <span class="text-blue-500 inline-block animate-bounce">L</span>
            <span class="text-red-500 inline-block -rotate-180 animate-spin"
              >o</span
            >
            <span
              class="text-yellow-500 inline-block scale-x-[-1] animate-pulse"
              >l</span
            >
            <span
              class="text-blue-500 inline-block rotate-12 hover:rotate-180 transition-transform"
              >G</span
            >
            <span
              class="text-green-500 inline-block -rotate-45 hover:scale-150 transition-transform"
              >O</span
            >
            <span class="text-red-500 inline-block scale-y-[-1] animate-bounce"
              >e</span
            >
          </span>
        </h1>
        ...
        <div class="mb-6 relative">
          <div class="relative">
            <input
              ...
              class="text-slate-700 w-full px-12 py-3 rounded-full border border-gray-200 hover:shadow-md focus:shadow-md transition-shadow duration-200 outline-none"
            />
            <svg
      ...
      <div v-else-if="searchResults.hits.length > 0" class="space-y-6 pb-8">
        ...
        >
          <router-link
            :to="{ name: 'product-details', params: { id: result._id } }"
            class="block hover:bg-gray-50 transition duration-150 ease-in-out rounded-lg p-4"
          >
            <h2
        ...
        <div v-if="totalPages > 1" class="flex justify-center mt-8">
          <div class="inline-flex items-center justify-center">
            <button
              ...
              class="text-[#1a0dab] hover:underline mr-4 flex items-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5 mr-1"
            ...
            <button
              ...
              @click="handlePageChange(currentPage + 1)"
              class="text-[#1a0dab] hover:underline ml-4 flex items-center"
            >
              Next
              <svg
                xmlns="http://www.w3.org/2000/svg"
